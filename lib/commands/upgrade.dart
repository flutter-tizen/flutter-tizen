...
class TizenUpgradeCommandRunner extends UpgradeCommandRunner {
  ...
  Future<FlutterCommandResult> run({
    ...
    if (!await TizenChannelCommand.isFlutterSdkClean(_gitWrapper, workingDirectory!)) {
      if (!force) {
        throwToolExit(
          'The vendored Flutter SDK in '
          '${globals.fs.path.join(workingDirectory!, 'flutter')} has '
          'uncommitted changes.\nCommit or stash them first, or re-run with '
          '--force to discard them and upgrade anyway.',
        );
      }
      await TizenChannelCommand.resetFlutterSdk(_gitWrapper, workingDirectory!);
    }

...
class TizenChannelCommand extends ChannelCommand {
  ...
  Future<void> _switchRelease(String target) async {
    ...
    if (!await isFlutterSdkClean(_gitWrapper, repoRoot)) {
      if (!boolArg('force')) {
        throwToolExit(
      ...
      await resetFlutterSdk(_gitWrapper, repoRoot);
    }
  ...
  /// Discards the vendored SDK's uncommitted changes for a forced switch.
  /// The bootstrap resets the SDK only when the pinned Flutter version
  /// changes, so between releases pinning the same version the changes
  /// would survive. `clean -df` (not `-xdf`) spares ignored files such as
  /// `bin/cache`, matching the scope [isFlutterSdkClean] guards.
  static Future<void> resetFlutterSdk(Git git, String repoRoot) async {
    final String flutterDir = globals.fs.path.join(repoRoot, 'flutter');
    globals.printStatus('Discarding local changes in the vendored Flutter SDK...');
    try {
      await git.run(
        <String>['reset', '--hard'],
        throwOnError: true,
      ...
      throwToolExit(
        'Could not discard the local changes in the vendored Flutter SDK in '
        '$flutterDir.\n${e.message}',
      );
    }
  }
